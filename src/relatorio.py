...
Diferente dos agentes 1/2/3 (que trabalham reclamação a reclamação), o
dashboard/relatório gerencial só faz sentido olhando o lote inteiro —
por isso vive fora do grafo, é chamado uma vez depois que todas as
reclamações já passaram pelos agentes (ver main.py).
"""
...
def exportar_csv(registros: list[dict], caminho: str) -> None:
    ...
    with destino.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for r in registros:
            parecer = r.get("parecer_risco") or {}
            writer.writerow({
                "id": r["id"],
                "canal": r["canal"],
                "categoria": r["analise"]["categoria"],
                "produto": r["analise"]["produto"],
                "sentimento": r["analise"]["sentimento"],
                "urgencia": r["analise"]["urgencia"],
...
def exportar_markdown(dashboard: dict, caminho: str) -> None:
    ...
    if dashboard["reclamacoes_criticas"]:
        ...
        linhas.append("|---|---|---|---|---|")
        for r in dashboard["reclamacoes_criticas"]:
            parecer = r.get("parecer_risco") or {}
            linhas.append(
                f"| {r['id']} | {r['canal']} | {r['analise']['categoria']} | "
